...
Before we proceed further, it's important to take snapshots of all your virtual machines. This way, if you break something during testing (which you should never be afraid of), you can always restore to a known good state.

In VirtualBox, right-click each VM and select **Snapshots**, then **Take Snapshot**. Give each snapshot a meaningful name like "AD Lab - Post Domain Setup".

![VM Snapshots](/img/VMSnapshots.png)
